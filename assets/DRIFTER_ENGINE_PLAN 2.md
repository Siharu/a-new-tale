...
> **SESSION UPDATE (JULY 4, 2026 — SPRITE SYSTEM, GAME RUNTIME, AND ASSET AUDIT):**
> A full repo scan against the running zip reveals significant work completed that was not yet in this doc. Recording everything discovered so it doesn't get rebuilt or lost to context drift.
>
> **NEW: `src/render/SpriteAnimator.ts` (208 lines) — not previously documented:**
> - `DIRECTIONS` const map: 8-direction vectors (NORTH, NORTHEAST, EAST, SE, SOUTH, SW, WEST, NW) + IDLE
> - `Direction` type exported
> - `vectorToDirection(x, y): Direction` — maps a 2D input vector to the nearest 8-way direction bucket using atan2 + 45° partitioning; returns IDLE when magnitude < 0.1
> - `SpriteAnimator` class: loads PNG sprite sheets per direction from `assets/characters/drifter/base/` (north.png, north-east.png, east.png, etc.), THREE.js `Sprite`-based rendering, frame stepping for walk animation, idle frame fallback. Tied to `IsometricRenderer`'s entity layer.
>
> **NEW: `src/ui/GameRuntime.ts` (436 lines) — not previously documented:**
> - Imported by `home-screen.ts`, wires the full play-mode scene: `IsometricRenderer` + `SkySystem` + `SpriteAnimator` + husk mesh rendering + zone-to-scene coordinate transform
> - `worldToScene()` helper: maps world grid coordinates to Three.js scene space using a `SCENE_HALF_EXTENT = 6` normalised box
> - `HUSK_COLOR` map: per-`HuskType` hex colors (behavioral/visual read — no real mechanism), used to colorize BoxGeometry placeholder husk meshes until real isometric husk sprites are wired in
> - `disposeObject3D()`: recursive geometry+material disposal for clean zone unload
> - Handles drifter sprite update each frame (calls `SpriteAnimator.update(direction, isMoving)`)
> - Zone buildings rendered via `SVGBuildingFactory` → `TextureCache` pipeline, not placeholders
> - **The one remaining `BoxGeometry` in this file is the player-drifter marker (intentional placeholder until SpriteAnimator takes full ownership of the drifter visual)**
>
> **`src/ui/home-screen.ts` has grown to 1,443 lines** (doc previously said 1,382 — it was updated post-doc).
>
> **ASSETS NOW ON DISK (doc previously said "no real assets yet" — this is out of date):**
> - `assets/characters/drifter/base/` — all 8 directional PNGs present: `north.png`, `north-east.png`, `east.png`, `south-east.png`, `south.png`, `south-west.png`, `west.png`, `north-west.png`. `SpriteAnimator.ts` loads these. Walk-cycle animation requires spritesheets (multi-frame) — current PNGs are single-frame stills; animation stubs to idle until real walk sheets arrive.
> - `assets/characters/husks/isometric/` — 6 husk types present with full 8-direction sets: `skoth/`, `noire/`, `jawie-bulky/`, `jawie-slim/`, `ghuul/`, `Pale-node-cultist/`. Each folder has north.png through west.png. **Missing: glowbubs, whites, oldbones, disabled, bloaters, aquatic** (3 of 9 Husk types + all Infected + Ghuul sprite variants not yet on disk — `GameRuntime.ts` falls back to `BoxGeometry` + `HUSK_COLOR` for these).
> - `assets/characters/drifter/portraits/` — handler + swat_1/swat_2/swat_3 portrait packs with emotion variants (aggression, calm, sadness, smile, special, talk) present. Gangster mob sprites also on disk (Gangsters_1/2/3 with full animation frames).
> - `assets/items/` — 100 item icon PNGs (1.png–100.png) present.
> - `assets/weapons/meleeweps/` — 50 melee weapon icons present.
> - `assets/tiles/ground/` and `assets/tiles/ground_sliced/` — tile assets present including the 5 confirmed ground tiles (grass_a, dirt_a, stone_basic, stone_rocky_grey, forest_a).
> - `assets/ui/fonts/gameicons/` — full SVG icon set present (200+ icons covering weapons, stats, UI glyphs).
> - `assets/ui/menubackground/` — 4 background sets (background 1–4), each with layered PNGs (1.png, 2.png, 3.png, orig.png, orig_big.png).
>
> **BUILD STATUS (July 4, 2026):** `node node_modules/typescript/bin/tsc --noEmit` → zero errors. All 11 render systems + 10 gameplay systems + 2 UI systems compile clean.
>
> **OPEN NEXT TASK — the one concrete unwired gap remaining:**
> `ZoneStreamer.onLoad` / `onUnload` callbacks are NOT wired from `home-screen.ts`'s `startRun()` into `GameRuntime`. Zone streaming runs in memory (zones load/unload from `WorldGenerator`) but Three.js scene objects (buildings, husks, tile geometry) are NOT added/removed when the player crosses zone boundaries. Fix: pass `onLoad(zone, isCenter)` → `GameRuntime.loadZone(zone)` and `onUnload(zoneID)` → `GameRuntime.unloadZone(zoneID)` from `startRun()` into `GameplayEngine`'s `zoneStreamerCallbacks` option. Once wired, multi-zone exploration works end-to-end.
>
> **SECONDARY GAPS (not blockers for first playable, but next after zone streaming):**
> - Drifter visual: `SpriteAnimator` loads correct PNGs but walk-cycle frames are stills — need multi-frame spritesheets for walk animation. Until then idle sprite displays even when moving.
> - Husk visuals: 3 husk types (glowbubs, whites, oldbones, disabled, bloaters, aquatic) have no isometric sprites — `GameRuntime` uses color-coded `BoxGeometry` fallback. Not a code problem, an asset gap.
> - `menu.html` nav buttons are not yet routed to real `setMode()`/`startRun()` in `home-screen.ts` — still toggling `.active` state only. This was intentional (visual mockup first), but needs wiring before public build.
> - `Archives` and `Credits` nav items still use placeholder overlays in `home-screen.ts`.
