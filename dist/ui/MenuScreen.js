...
export function loadSettings() {
    const defaults = { difficulty: 3, volume: 0.65, showHints: true };
    try {
...
export class MenuScreen {
    constructor(rootEl, audio, callbacks) {
    ...
    render() {
        ...
        if (showAtmosphere)
            this.root.appendChild(this.buildScanlines(p.scanlineOpacity));
        if (showAtmosphere)
        ...
        switch (mode) {
            ...
            case 'exploration':
            case 'settings':
                layout.appendChild(this.buildSplitLayout(mode));
                break;
            case 'loading':
                layout.appendChild(this.buildLoadingScreen());
                break;
            case 'briefing':
                layout.appendChild(this.buildBriefingScreen());
                break;
        }
    ...
    buildBackground(skyFilter) {
        ...
        if (rainStates.includes(this.wrongnessState)) {
            const rainOpacity = this.wrongnessState === WrongnessState.RAINY ? 0.18 :
            ...
            const rainColor = (this.wrongnessState === WrongnessState.DIFFERENT || this.wrongnessState === WrongnessState.ANOTHER_SKY)
                ? 'rgba(180, 40, 40, 0.6)'
        ...
        const orangeMoonStates = [WrongnessState.UNKNOWN];
    ...
    buildSplitLayout(mode) {
        ...
        if (mode === 'story')
            right.appendChild(this.buildStoryPanel());
        if (mode === 'exploration')
            right.appendChild(this.buildExplorationPanel());
        if (mode === 'settings')
            right.appendChild(this.buildSettingsPanel());
        const bar = el('div');
    ...
    makeSlider(label, min, max, value, onChange) {
    ...
    buildBriefingScreen() {
        ...
        for (const [lbl, val, warn] of stats) {
            const l = el('span', { color: 'rgba(140,165,190,0.6)' });
            l.textContent = lbl;
            const v = el('span', { color: warn ? '#ff7a6a' : 'rgba(220,235,248,0.9)' });
            v.textContent = val;
            grid.appendChild(l);
            grid.appendChild(v);
        }
    ...
    buildSignalMeter(percent) {
        ...
        if (percent >= 75) {
            ...
            meter.style.gap = '4px';
            meter.style.alignItems = 'center';
            for (const [anim, width] of [['sig-morse-long', 28], ['sig-morse-short', 18], ['sig-morse-dot', 6]]) {
                const dash = el('span');
...
//# sourceMappingURL=MenuScreen.js.map