...
export class RainSystem {
    constructor(options = {}) {
        ...
        this.splashTimer = 0;
        this.splashInterval = 0.04; // seconds between spawn attempts
        ...
        this.regularPoints = this.buildRainPoints(this.regularCount, 
        ...
        /* opacity */ 0.55, 
        /* seed */ 0x1a2b3c);
        this.regularGeo = this.regularPoints.geometry;
        this.regularMat = this.regularPoints.material;
        this.obsediaPoints = this.buildRainPoints(this.obsediaCount, 
        ...
        /* opacity */ 0.82, 
        /* seed */ 0xdeadbeef);
        this.obsediaGeo = this.obsediaPoints.geometry;
        this.obsediaMat = this.obsediaPoints.material;
        // Splash pool
    ...
    // ── Internal ───────────────────────────────────────────────────────────────
    resolveMode(state) {
        if (state.obsediaActive && state.obsediaIntensity > 0.05)
            return 'obsedia';
        ...
        if (isRainWeather || isRainWrongness)
            return 'regular';
        return 'none';
    }
    applyMode(mode, obsediaIntensity) {
        if (mode === 'obsedia') {
            this.targetOpacity = 0.55 + obsediaIntensity * 0.45;
            // Obsedia splashes are near-black
            this.splashMat.uniforms.uColor.value.set(0x0a0a0e);
        }
        else if (mode === 'regular') {
            this.targetOpacity = 0.7;
            this.splashMat.uniforms.uColor.value.set(0x8aabcc);
        }
        else {
            this.targetOpacity = 0;
        }
    ...
    updateSplashes(dt, mode, obsediaIntensity) {
        ...
        // upload to GPU
        this.splashGeo.getAttribute('aLife').needsUpdate = true;
        this.splashGeo.getAttribute('aSize').needsUpdate = true;
        this.splashGeo.getAttribute('position').needsUpdate = true;
        const anySplashAlive = this.splashPool.some(s => s.life < 1);
        this.splashPoints.visible = anySplashAlive;
    }
    buildRainPoints(count, color, speed, angleX, angleZ, streakH, streakW, opacity, seed) {
...
//# sourceMappingURL=RainSystem.js.map