...
export interface GodRayLayerOptions {
  ...
  exposure?: number;       // default 0.35 — with the sun-disc fix, only a small screen area is ever bright, so this can run higher than the old whole-dome-exempt version could safely support
  decay?: number;          // default 0.94
  density?: number;        // default 0.7   — shaft length; higher reads fine now that the source is a small disc, not the whole sky
  weight?: number;         // default 0.35
  rayColor?: THREE.Color;  // default warm near-white, overridden per-bake from SkySystem's sunMoon color
  sunRadius?: number;      // default 0.06 — disc radius in 0..1 screen-space units
}
...
export class GodRayLayer {
  ...
  // Small bright disc injected into the occlusion buffer at the light's
  // screen position — see SUN_DISC_FRAGMENT doc comment above. This
  // replaced an earlier approach that exempted the entire sky dome from
  // the black-silhouette pass; a screen-filling bright dome made almost
  // every pixel of the occlusion mask read as "light source," so the
  // radial blur below washed the whole frame with a uniform tint instead
  // of producing actual shafts.
  private sunDiscMaterial: THREE.ShaderMaterial;
  private sunDiscScene: THREE.Scene;
  private sunDiscCamera: THREE.OrthographicCamera;
  ...
  setRayColor(color: THREE.Color): void {
    ...
    (this.sunDiscMaterial.uniforms.sunColor.value as THREE.Color).copy(color);
  }
  ...
  renderOcclusion(
    ...
    (this.sunDiscMaterial.uniforms.sunScreenPos.value as THREE.Vector2).copy(lightScreenPos);
